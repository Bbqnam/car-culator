...
export function getDisplayModelName(brand: string, model: string): string {
  const canonicalBrand = canonicalizeDatabaseBrand(brand);
  const cleanedModel = model.replace(/\s+/g, " ").trim();
  if (!cleanedModel) return cleanedModel;

  const brandPrefix = new RegExp(`^${canonicalBrand.replace(/[.*+?^${}()|[\]\\]/g, "\\$&")}\\s+`, "i");
  return cleanedModel.replace(brandPrefix, "").trim();
}
